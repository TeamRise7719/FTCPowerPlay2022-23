...
/**
 * Created by Sean Cardosi on 9/16/22.
 */
public class SeansSynchronousPID {
     ...
     */
    public SeansSynchronousPID(double Kp, double Ki, double Kd) {
        m_P = Kp;
        m_I = Ki;
        m_D = Kd;
    }
    ...
    public double calculateUseError(double error) {
        m_error = error;
        if (m_continuous) {
            if (Math.abs(m_error) > (m_maximumInput - m_minimumInput) / 2) {
                if (m_error > 0) {
